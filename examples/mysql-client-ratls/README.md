...
The application uses a structured directory layout with different security levels:

```
/app/
├── code/                    # Code directory (mounted at /app/code)
│   ├── mysql-client.js      # Main client script (trusted_files - integrity checked)
│   ├── package.json         # NPM configuration (trusted_files - integrity checked)
│   ├── node_modules/        # NPM dependencies (trusted_files - integrity checked)
│   └── data/                # Encrypted partition for runtime data
│       └── (runtime files)  # Sensitive data generated by code (encrypted)
└── wallet/                  # Encrypted partition for keys
    └── mysql-client-keys/   # Client private keys (encrypted)
```
...
|------|------------|-------------|
| `/app/code/mysql-client.js` | trusted_files | Integrity-checked at enclave load time, cannot be modified |
| `/app/code/package.json` | trusted_files | Integrity-checked at enclave load time |
| `/app/code/node_modules/` | trusted_files | All dependencies integrity-checked |
| `/app/code/data/` | encrypted | Encrypted partition for sensitive runtime data |
| `/app/wallet/` | encrypted | Encrypted partition for private keys |
| `/var/lib/mysql-client-ssl/` | allowed_files | Writable directory for certificates |

### Encrypted Partitions

Encrypted partitions use SGX sealing with `_sgx_mrenclave` key, meaning:
- Data is encrypted with a key derived from the enclave measurement
- Only the same enclave code can decrypt the data
- Data persists across container restarts (if volume is mounted)
- Provides confidentiality for sensitive runtime data
...
- **Code Integrity**: Application code is protected via trusted_files (integrity-checked at load time)
- **Runtime Data Protection**: Sensitive runtime data is stored in encrypted partition (`/app/code/data/`)
